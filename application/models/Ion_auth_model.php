...
class Ion_auth_model extends CI_Model
{
    ...
    public function clear_forgotten_password_code($code)
    {
        ...
        if ($this->db->count_all_results($this->tables['tb_users']) > 0) {
            $data = array(
                'forgotten_password_code' => NULL,
                'forgotten_password_time' => NULL
            );

            $this->db->update($this->tables['tb_users'], $data, array('forgotten_password_code' => $code));
    ...
    public function reset_password($identity, $new)
    {
        ...
        $this->trigger_events('extra_where');
        $this->db->update($this->tables['tb_users'], $data, array($this->identity_column => $identity));
    ...
    public function change_password($identity, $old, $new)
    {
        ...
        if ($old_password_matches === TRUE) {
            ...
            $successfully_changed_password_in_db = $this->db->update($this->tables['tb_users'], $data, array($this->identity_column => $identity));
    ...
    public function username_check($username = '')
    {
        ...
        return $this->db->where('username', $username)
            ...
            ->order_by("id", "ASC")
            ->limit(1)
            ->count_all_results($this->tables['tb_users']) > 0;
    }
    ...
    public function forgotten_password($identity)
    {
        ...
        $this->db->update($this->tables['tb_users'], $update, array($this->identity_column => $identity));
    ...
    public function register($identity, $password, $email, $additional_data = array(), $groups = array())
    {
        ...
        // $query = $this->db->get_where($this->tables['tb_groups'], array('name' => $this->config->item('default_group', 'ion_auth')), 1)->row();
        // // var_dump($query);
        // // die();
        ...
        $user_data = array_merge($this->_filter_data($this->tables['tb_users'], $additional_data), $data);

        $this->trigger_events('extra_set');

        $this->db->insert($this->tables['tb_users'], $user_data);

        $id = $this->db->insert_id();

        $id_kampus = $user_data['id_kampus'];

        // var_dump($cektypeKampus->id_instansi_type);
        // die();
        $aksesinstasi = [
            'id' => '',
            'id_users' => $id,
            'id_instansi' => $id_kampus,
        ];
        $this->db->insert('tb_users_akses_instansi', $aksesinstasi);
        $cektypeKampus = $this->db->get_where('mst_kampus', array('id' => $id_kampus))->row();
        if ($cektypeKampus->id_instansi_type == 1) {
            $user_group = [
                'id' => '',
                'user_id' => $id,
                'group_id' => '2',
            ];
            $this->db->insert('tb_users_groups', $user_group);
        } else if ($cektypeKampus->id_instansi_type == 2) {
            $user_group = [
                'id' => '',
                'user_id' => $id,
                'group_id' => '4',
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select($this->identity_column . ', email, id, password,username,company,first_name,last_name, active, last_login')
            ->where($this->identity_column, $identity)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['tb_users']);
    ...
    function get_attempts_num($identity)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            $qres = $this->db->get($this->tables['tb_login_attempts']);
    ...
    public function get_last_attempt_time($identity)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            if ($this->config->item('track_login_ip_address', 'ion_auth'))
                ...
                $this->db->or_where('login', $identity);
            $qres = $this->db->get($this->tables['tb_login_attempts'], 1);
    ...
    public function increase_login_attempts($identity)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            $ip_address = $this->_prepare_ip($this->input->ip_address());
            return $this->db->insert($this->tables['tb_login_attempts'], array('ip_address' => $ip_address, 'login' => $identity, 'time' => time()));
        }
        return FALSE;
    }
    ...
    public function clear_login_attempts($identity, $expire_period = 86400)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            return $this->db->delete($this->tables['tb_login_attempts']);
        }
        return FALSE;
    }
    ...
    public function users($groups = NULL)
    {
        ...
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            ...
            $this->db->select(array(
                $this->tables['tb_users'] . '.*',
                $this->tables['tb_users'] . '.id as id',
                $this->tables['tb_users'] . '.id as user_id'
            ));
        }
        ...
        if (isset($groups)) {
            ...
            if (isset($groups) && !empty($groups)) {
                $this->db->distinct();
                $this->db->join(
                    $this->tables['tb_users_groups'],
                    $this->tables['tb_users_groups'] . '.' . $this->join['tb_users'] . '=' . $this->tables['tb_users'] . '.id',
                    'inner'
                );
            }
            ...
            if (!empty($group_names)) {
                $this->db->join($this->tables['tb_groups'], $this->tables['tb_users_groups'] . '.' . $this->join['tb_groups'] . ' = ' . $this->tables['tb_groups'] . '.id', 'inner');
                $this->db->where_in($this->tables['tb_groups'] . '.name', $group_names);
            }
            if (!empty($group_ids)) {
                $this->db->{$or_where_in}($this->tables['tb_users_groups'] . '.' . $this->join['tb_groups'], $group_ids);
            }
        }
        ...
        $this->response = $this->db->get($this->tables['tb_users']);
    ...
    public function user($id = NULL)
    {
        ...
        $this->limit(1);
        $this->order_by($this->tables['tb_users'] . '.id', 'desc');
        $this->where($this->tables['tb_users'] . '.id', $id);
    ...
    {
        $this->trigger_events('get_tb_users_group');

        // if no id was passed use the current users id
        $id || $id = $this->session->userdata('user_id');

        return $this->db->select($this->tables['tb_users_groups'] . '.' . $this->join['tb_groups'] . ' as id, ' . $this->tables['tb_groups'] . '.name, ' . $this->tables['tb_groups'] . '.description')
            ->where($this->tables['tb_users_groups'] . '.' . $this->join['tb_users'], $id)
            ->join($this->tables['tb_groups'], $this->tables['tb_users_groups'] . '.' . $this->join['tb_groups'] . '=' . $this->tables['tb_groups'] . '.id')
            ->get($this->tables['tb_users_groups']);
    }
    ...
    public function add_to_group($group_ids, $user_id = false)
    {
        ...
        foreach ($group_ids as $group_id) {
            if ($this->db->insert($this->tables['tb_users_groups'], array($this->join['tb_groups'] => (float) $group_id, $this->join['tb_users'] => (float) $user_id))) {
    ...
    public function remove_from_group($group_ids = false, $user_id = false)
    {
        ...
        if (!empty($group_ids)) {
            ...
            foreach ($group_ids as $group_id) {
                $this->db->delete($this->tables['tb_users_groups'], array($this->join['tb_groups'] => (float) $group_id, $this->join['tb_users'] => (float) $user_id));
        ...
        // otherwise remove user from all groups
            ...
            if ($return = $this->db->delete($this->tables['tb_users_groups'], array($this->join['tb_users'] => (float) $user_id))) {
    ...
    public function groups()
    {
        ...
        $this->response = $this->db->get($this->tables['tb_groups']);
    ...
    public function group($id = NULL)
    {
        ...
        if (isset($id)) {
            $this->where($this->tables['tb_groups'] . '.id', $id);
        }
    ...
    public function update($id, array $data)
    {
        ...
        $data = $this->_filter_data($this->tables['tb_users'], $data);
        ...
        $this->trigger_events('extra_where');
        $this->db->update($this->tables['tb_users'], $data, array('id' => $user->id));
        $datainstansi = [
            'id_instansi' => $data['id_kampus'],
        ];
        $this->db->update(
            'tb_users_akses_instansi',
            $datainstansi,
        ...
        if ($cektypeKampus->id_instansi_type == 1) {
            ...
            $this->db->update('tb_users_groups', $user_group, array('user_id' => $user->id));
        } else if ($cektypeKampus->id_instansi_type == 2) {
            $user_group = [
            ...
            $this->db->insert('tb_users_groups', $user_group, array('user_id' => $user->id));
        }
    ...
    public function delete_user($id)
    {
        ...
        $this->db->delete($this->tables['tb_users'], array('id' => $id));
        $this->db->delete('tb_users_akses_instansi', array('id_users' => $id));
    ...
    public function update_last_login($id)
    {
        ...
        $this->db->update($this->tables['tb_users'], array('last_login' => date("Y-m-d H:i:s")), array('id' => $id));
    ...
    public function remember_user($id)
    {
        ...
        $this->db->update($this->tables['tb_users'], array('remember_code' => $salt), array('id' => $id));
    ...
    public function login_remembered_user()
    {
        ...
        // get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, email,username,company,first_name,last_name,last_login')
    ...
    public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
    {
        ...
        $existing_group = $this->db->get_where($this->tables['tb_groups'], array('name' => $group_name))->num_rows();
        if ($existing_group !== 0) {
        ...
        if (!empty($additional_data))
            $data = array_merge($this->_filter_data($this->tables['tb_groups'], $additional_data), $data);

        $this->trigger_events('extra_group_set');

        // insert the new group
        $this->db->insert($this->tables['tb_groups'], $data);
    ...
    public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
    {
        ...
        if (!empty($group_name)) {
            ...
            // bail if the group name already exists
            $existing_group = $this->db->get_where($this->tables['tb_groups'], array('name' => $group_name))->row();
        ...
        $group = $this->db->get_where($this->tables['tb_groups'], array('id' => $group_id))->row();
        ...
        $this->db->update($this->tables['tb_groups'], $data, array('id' => $group_id));
    ...
    public function delete_group($group_id = FALSE)
    {
        ...
        $this->db->delete($this->tables['tb_users_groups'], array($this->join['tb_groups'] => $group_id));
        // remove the group itself
        $this->db->delete($this->tables['tb_groups'], array('id' => $group_id));
